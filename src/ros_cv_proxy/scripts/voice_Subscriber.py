...
class VoiceIO:
  ...
  def releaseName(self):
    if self.doneName is not None:
      self.doneName = list(filter(lambda x: time.time() - x['ts'] < self.releaseTime, self.doneName))
  ...
  def voiceback(self, data):
      ...
      if data.OrderFinish and self.name != '':
        self.doneName.append(dict(ts=time.time(), name=self.name))
        self.releaseName()
